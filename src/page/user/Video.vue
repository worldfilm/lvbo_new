  ...
  <div class="video-c">
    ...
    <div class="myvideo-upload" v-show='NoHaveVideo'>
      <div class="myvideo-upload-desc">
        <a @click="uploadvideo" class="myvideo-upload-desc-img"><i class="iconfont icon-ziyuan" ></i>我要上传</a>
      </div>
    </div>
    <div class="myvideo-videolist-box" v-show='NoHaveVideo'>
      <div class="myvideo-nothing">
        <span class="iconfont icon-tishi"></span> 暂无记录,去
...
export default {
  ...
  created() {
    ...
    let page = this.pangnum;
    let page_size = this.page_size;
    let params = {
      page,
      page_size
    };
    this.$http.get("/api/user/getMyVideo", params).then(data => {
      if (data.status == 0) {
