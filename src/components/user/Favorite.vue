...
<div class="favorite">
  ...
  <div class="myvideo" v-show='HaveVideo'>
    <ul class="videos-cont">
      <li v-for="(item,idx) in list" class="item" :key="idx" @click='openvideo(item)'>
        ...
        <p><button type="button" name="button" @click='deletvideo'>取消收藏</button></p>
      </li>
    </ul>
  </div>
</div>
...
export default {
  ...
  created() {
    ...
    this.$http.get("/api/user/list/collect", params).then(data => {
      console.log(data);
      data.data.list = [];
      if (data.status == 0) {
